...
#.  --dll=nom_du_fichier_dll                     : Spécifie le nom du fichier .dll de l'application .Net Core (obligatoire)
#.  --internal-port=port_interne                 : Spécifie le port interne sur lequel l'application .Net Core écoute (obligatoire)
#.  --environment=Environnement_de_l_application : Spécifie l'environnement de l'application (optionnel, par défaut "Production")
...
if [ "$#" -lt 3 ] || [ "$#" -gt 8 ]; then
    echo "Usage: $0 --domain=domain.com --dll=my.dll [--environment=envrionment] [--lock-username=user --lock-password=pass] [--ftp-username=user --ftp-password=pass]"
    exit 1
fi
...
if [[ "$2" == "--dll="* ]]; then
    ...
    echo "Le deuxième argument doit être --dll=my.dll"
    exit 1
fi

# port
if [[ "$3" == "--internal-port="* ]]; then
    INTERNAL_PORT="${3#--internal-port=}"
else
    echo "Le troisième argument doit être --internal-port=5000"
    exit 1
fi

# environment
ENVIRONMENT="Production"
if [[ "$4" == "--environment="* ]]; then
...
if [[ $4 == --lock-username=* ]] && [[ "$5" == --lock-password=* ]]; then
    LOCK_SITE=true
    USERNAME="${4#--lock-username=}"
    PASSWORD="${5#--lock-password=}"
elif [[ $5 == --lock-username=* ]] && [[ "$6" == --lock-password=* ]]; then
    LOCK_SITE=true
    USERNAME="${5#--lock-username=}"
    PASSWORD="${6#--lock-password=}"
elif [[ "$6" == --lock-username=* ]] && [[ "$7" == --lock-password=* ]]; then
    LOCK_SITE=true
    USERNAME="${6#--lock-username=}"
    PASSWORD="${7#--lock-password=}"
elif [[ "$7" == --lock-username=* ]] && [[ "$8" == --lock-password=* ]]; then
    LOCK_SITE=true
    USERNAME="${7#--lock-username=}"
    PASSWORD="${8#--lock-password=}"
fi

#ftp
FTP_ENABLED=false

if [[ "$4" == --ftp-username=* ]] && [[ "$5" == --ftp-password=* ]]; then
    FTP_ENABLED=true
    FTP_USER="${4#--ftp-username=}"
    FTP_PASSWORD="${5#--ftp-password=}"
elif [[ "$5" == --ftp-username=* ]] && [[ "$6" == --ftp-password=* ]]; then
    FTP_ENABLED=true
    FTP_USER="${5#--ftp-username=}"
    FTP_PASSWORD="${6#--ftp-password=}"
elif [[ "$6" == --ftp-username=* ]] && [[ "$7" == --ftp-password=* ]]; then
    FTP_ENABLED=true
    FTP_USER="${6#--ftp-username=}"
    FTP_PASSWORD="${7#--ftp-password=}"
elif [[ "$7" == --ftp-username=* ]] && [[ "$8" == --ftp-password=* ]]; then
    FTP_ENABLED=true
    FTP_USER="${7#--ftp-username=}"
    FTP_PASSWORD="${8#--ftp-password=}"
fi
